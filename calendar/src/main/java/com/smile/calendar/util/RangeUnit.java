...
public abstract class RangeUnit extends CalendarUnit {
    ...
    public int getFirstWeek(@Nullable LocalDate firstDayOfMonth) {
        ...
        LocalDate date;
        if (mMinDate != null && mMinDate.isAfter(from)) {
            date = mMinDate;
        } else {
    ...
    @Nullable
    public abstract LocalDate getFirstDateOfCurrentMonth(@NonNull LocalDate currentMonth);

    public int getWeekInMonth(@Nullable LocalDate date) {
        if (date != null) {
