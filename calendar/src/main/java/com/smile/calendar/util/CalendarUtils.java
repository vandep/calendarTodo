...
//                    String address = eventCursor.getString(eventCursor.getColumnIndex("event_location"));//日程事件的位置，发现没有这个字段
    ...
    public static void writeCalendarEvent(Activity activity, EventModel eventModel) {
        if (requestPermission(activity)) {
            ContentResolver contentResolver = activity.getContentResolver();
            String calId = "";
            Cursor userCursor = contentResolver.query(Uri.parse(CalendarUtils.calanderURL), null,
                    null, null, null);
            ...
            event.put(CalendarContract.Events.TITLE, eventModel.mName);
            event.put(CalendarContract.Events.DESCRIPTION, eventModel.mContent);
            ...
            Uri newEvent = contentResolver.insert(Uri.parse(CalendarUtils.calanderEventURL), event);
            long id = Long.parseLong(newEvent.getLastPathSegment());
            ContentValues values = new ContentValues();
            values.put(CalendarContract.Reminders.EVENT_ID, id);
            //提前10分钟有提醒
            values.put(CalendarContract.Reminders.MINUTES, 10);
            contentResolver.insert(Uri.parse(CalendarUtils.calanderRemiderURL), values);
            Log.e("pcm", "writeCalendarEvent 插入事件成功!!!");
        }
    }
}
